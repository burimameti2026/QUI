...
@Component({
  ...
  template: `<qai-page-header
    ...
    <div class="metrics compact">
      <article><span>Open value</span><strong>{{ money(openValue) }}</strong></article>
      <article><span>Won</span><strong>{{ money(wonValue) }}</strong></article>
      <article><span>Open deals</span><strong>{{ openCount }}</strong></article>
    </div>
    <section class="panel table-wrap">
      <div class="data-state" *ngIf="loading">Loading opportunities…</div>
      <div class="data-state" *ngIf="!loading && !error && !rows.length"><b>No opportunities yet</b><span>Convert a qualified lead or add an opportunity manually.</span><button class="primary" (click)="open()">Create opportunity</button></div>
      <table *ngIf="!loading && rows.length">
        <thead><tr><th>Opportunity</th><th>Value</th><th>Sales process</th><th>Status</th><th>Expected close</th><th>Automation influence</th><th></th></tr></thead>
        <tbody><tr *ngFor="let x of rows">
          <td><b>{{ x.name }}</b></td><td>{{ money(x.amount) }}</td>
          <td><span class="stage-assignment" [class.unassigned]="!x.pipelineStageId">{{ stageLabel(x) }}</span></td>
          <td><span class="pill">{{ status(x.status) }}</span></td><td>{{ x.expectedCloseUtc | date: "mediumDate" }}</td>
    ...
    <qai-modal [open]="show" [title]="form.id ? 'Edit opportunity' : 'New opportunity'" (close)="show = false">
      <form class="form" (ngSubmit)="save()">
        ...
        <div class="form2"><label>Amount<input type="number" [(ngModel)]="form.amount" name="amount" /></label><label>Status<select [(ngModel)]="form.status" name="status"><option [ngValue]="0">Open</option><option [ngValue]="1">Won</option><option [ngValue]="2">Lost</option></select></label></div>
        <div class="form2"><label>Sales pipeline<select [(ngModel)]="formPipelineId" name="pipeline" (ngModelChange)="choosePipeline($event)"><option value="">Assign later</option><option *ngFor="let pipeline of pipelines" [value]="pipeline.id">{{pipeline.name}}{{pipeline.isDefault ? ' · Default' : ''}}</option></select></label><label>Pipeline stage<select [(ngModel)]="form.pipelineStageId" name="stage" [disabled]="!formPipelineId"><option [ngValue]="undefined">Choose stage</option><option *ngFor="let stage of formStages" [value]="stage.id">{{stage.name}} · {{stage.probability}}%</option></select><small *ngIf="!pipelines.length">Create a pipeline first, then assign this deal.</small></label></div>
        ...
        <label>Expected close<input type="date" [(ngModel)]="closeDate" name="close" /></label>
        <footer><button type="button" (click)="show = false">Cancel</button><button class="primary" type="submit">Save opportunity</button></footer>
      </form>
    </qai-modal>`,
})
export class OpportunitiesPage implements OnInit {
  ...
  get openValue() { return this.rows.filter(x => this.status(x.status) === "Open").reduce((s, x) => s + (x.amount || 0), 0); }
  get wonValue() { return this.rows.filter(x => this.status(x.status) === "Won").reduce((s, x) => s + (x.amount || 0), 0); }
  get openCount() { return this.rows.filter(x => this.status(x.status) === "Open").length; }
