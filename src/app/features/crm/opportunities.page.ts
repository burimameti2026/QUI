...
@Component({
  ...
  template: `<main class="page">
    ...
    <section class="metric-grid"><article class="metric"><span class="metric-label">Open value</span><strong>{{ money(openValue) }}</strong><small>Current open opportunities</small></article><article class="metric"><span class="metric-label">Won</span><strong>{{ money(wonValue) }}</strong><small>Closed-won value</small></article><article class="metric"><span class="metric-label">Open deals</span><strong>{{ openCount }}</strong><small>Active opportunities</small></article></section>
    <section class="card">
      ...
      <div class="table" *ngIf="!loading && rows.length"><table><thead><tr><th>Opportunity</th><th>Value</th><th>Sales process</th><th>Status</th><th>Expected close</th><th>Automation influence</th><th></th></tr></thead><tbody><tr *ngFor="let x of rows"><td><b>{{ x.name }}</b></td><td>{{ money(x.amount) }}</td><td><span class="status" [class.unassigned]="!x.pipelineStageId">{{ stageLabel(x) }}</span></td><td><span class="status">{{ status(x.status) }}</span></td><td>{{ x.expectedCloseUtc | date: "mediumDate" }}</td><td><span [class.positive]="!!x.leadId">{{ x.leadId ? 'Qualified lead / campaign' : 'Manual opportunity' }}</span></td><td><button class="small" (click)="open(x)">Edit</button></td></tr></tbody></table></div>
    </section>
