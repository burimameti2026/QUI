...
@Component({
  ...
  template: `
    ...
    <section class="lead-workspace">
      ...
      <div class="lead-empty" *ngIf="!loading && !error && !visible.length"><strong>{{ rows.length ? 'No matching leads' : 'No leads available' }}</strong><span>{{ rows.length ? 'No leads match the current search or status filter.' : 'Create a lead or run the acquisition workflow.' }}</span><button class="primary" (click)="openCreate()">Create Lead</button></div>
      <qai-data-grid *ngIf="!loading && visible.length"><table><thead><tr><th class="check-col"><input type="checkbox" /></th><th>Customer</th><th>Company</th><th>Email</th><th>Status</th><th>Manager</th><th>Source</th><th>Score</th><th>Created</th><th>Actions</th></tr></thead><tbody><tr *ngFor="let x of visible"><td class="check-col"><input type="checkbox" /></td><td><div class="lead-person"><b>{{ x.intentSummary || 'New enquiry' }}</b></div></td><td>{{ x.company || x.source || '—' }}</td><td>{{ x.email || '—' }}</td><td><span class="lead-status" [class.hot]="x.score >= 80">{{ temperature(x) }}</span></td><td>{{ x.manager || '—' }}</td><td>{{ x.source || 'Search' }}</td><td>{{ x.score ?? '—' }}</td><td>{{ createdDate(x) }}</td><td><div class="directory-actions"><button class="small" (click)="qualify(x)">Qualify</button><button class="small primary" (click)="convert(x)">Opportunity</button></div></td></tr></tbody></table></qai-data-grid>
    </section>
