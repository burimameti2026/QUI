  ...
  template: `
    ...
    <section class="lead-workspace">
      <div class="lead-metrics">
        <article>
          <span>New leads</span><strong>{{ rows.length }}</strong><small>↗ 12% vs previous period</small>
          <span class="kpi-icon">♧</span><svg class="kpi-chart" viewBox="0 0 108 38" aria-hidden="true"><polyline points="0,29 14,15 27,24 39,18 51,26 65,11 78,20 91,8 108,3"/></svg>
        </article>
        <article>
          <span>Qualified leads</span><strong>{{ count(80, 101) }}</strong><small>↗ 4.2% vs previous period</small>
          <span class="kpi-icon">◎</span><svg class="kpi-chart" viewBox="0 0 108 38" aria-hidden="true"><polyline points="0,25 13,17 25,21 39,10 52,16 64,12 76,20 91,8 108,4"/></svg>
        </article>
        <article>
          <span>Hot leads</span><strong>{{ count(80, 101) }}</strong><small>Ready for sales follow-up</small>
          <span class="kpi-icon">◇</span><svg class="kpi-chart" viewBox="0 0 108 38" aria-hidden="true"><polyline points="0,30 13,23 25,27 38,16 51,21 65,13 77,17 91,7 108,12"/></svg>
        </article>
        <article>
          <span>Open value</span><strong>{{ money(total()) }}</strong><small>Estimated pipeline value</small>
          <span class="kpi-icon">€</span><svg class="kpi-chart" viewBox="0 0 108 38" aria-hidden="true"><polyline points="0,27 13,25 26,17 39,21 52,13 65,17 78,9 92,14 108,4"/></svg>
        </article>
      </div>

      <nav class="lead-tabs" aria-label="Lead filters">
        <button class="active" type="button">All</button><button type="button">Favorite</button><button type="button">New</button><button type="button">Assigned to me</button><button type="button">Overdue</button><button type="button">Hot</button>
      </nav>
      ...
      <div class="lead-toolbar">
        ...
        <select [(ngModel)]="temp" aria-label="Filter by temperature"><option value="">All statuses</option><option>Hot</option><option>Warm</option><option>Cold</option></select>
      ...
      <div class="lead-table-wrap" *ngIf="!loading && visible.length">
        <table class="lead-table">
          <thead><tr><th class="check-col"><input type="checkbox" aria-label="Select all" /></th><th>Lead</th><th>Company</th><th>Email</th><th>Status</th><th>Manager</th><th>Value</th><th>Actions</th></tr></thead>
          <tbody>
            <tr *ngFor="let x of visible">
              ...
              <td>{{ x.manager || '—' }}</td><td><b>{{ money(x.estimatedValue || 0) }}</b></td>
