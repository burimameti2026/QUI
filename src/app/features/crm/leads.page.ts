  ...
  template: `
    ...
    <section class="page">
      ...
      <qai-data-grid *ngIf="!loading && !error">
        <div class="toolbar">
          <label class="search"><span>⌕</span><input [(ngModel)]="q" placeholder="Search leads, companies, contacts..." /></label><button type="button" class="toolbar-button">▽ &nbsp;Filters</button><button type="button" class="toolbar-button">↕ &nbsp;Sort</button><select [(ngModel)]="temp"><option value="">All statuses</option><option>Hot</option><option>Warm</option><option>Cold</option></select><div class="grid-tools"><button type="button">⇧</button><button type="button">⇩</button><button type="button" (click)="load()">↻</button><div class="view-toggle"><button type="button" class="active">☷</button><button type="button">⊞</button></div></div>
        </div>
        <table>
