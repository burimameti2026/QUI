...
export class RenovaPromotionPage implements OnInit {
  ...
  planCreated=false; saving=false; error='';

  constructor(private api:ApiService, private route:ActivatedRoute) {}

  ngOnInit():void {
    this.api.get<Product[]>('renova/catalog/products').subscribe({
      next:products=>{ this.products=products; const requested=this.route.snapshot.queryParamMap.get('productId'); this.product=requested&&products.some(p=>p.id===requested)?requested:products[0]?.id||''; },
  ...
  createPlan():void {
    ...
    const create=(market:Market)=>this.api.post<any>('renova/catalog/promotion-plans',{
      catalogProductId:this.product,targetMarketId:market.id||null,
