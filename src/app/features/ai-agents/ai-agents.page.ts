  ...
  template: `<qai-page-header
    ...
    <div class="studio">
      <aside>
        ...
        <nav>
          <button [class.active]="tab === 'behavior'" (click)="tab = 'behavior'">Behavior</button
          ><button [class.active]="tab === 'tools'" (click)="tab = 'tools'">Tools & actions</button
          ><button [class.active]="tab === 'model'" (click)="tab = 'model'">Model</button
          ><button [class.active]="tab === 'guardrails'" (click)="tab = 'guardrails'">Guardrails</button>
        </nav>
      </aside>
      <section class="panel form" *ngIf="tab === 'behavior'">
        <h3>Agent behavior</h3>
        <div class="form2">
          <label>Name<input [(ngModel)]="agent.name" /></label><label>Role<input [(ngModel)]="agent.role" /></label>
        </div>
        <label>Instructions<textarea class="large" [(ngModel)]="agent.instructions"></textarea></label>
        <div class="form2">
          <label>Tone<select [(ngModel)]="agent.tone"><option>professional</option><option>friendly</option><option>concise</option></select></label><label>Languages<input [(ngModel)]="agent.languagesCsv" /></label>
        </div>
        <label class="checkline"><input type="checkbox" [(ngModel)]="agent.active" /> Agent active</label>
      </section>
      <section class="panel" *ngIf="tab === 'tools'">
        <h3>Tools & actions</h3><p>Actions available to the workflow runtime after tenant and permission checks.</p>
        <div class="tools"><article *ngFor="let t of tools"><i>⚡</i><div><b>{{ t }}</b><span>{{ desc(t) }}</span></div><span class="pill success">Enabled</span></article></div>
      </section>
      <section class="panel form" *ngIf="tab === 'model'">
        <h3>Model routing</h3><label>Primary model<select [(ngModel)]="agent.model"><option>local</option><option>gpt-5</option><option>azure-openai</option></select></label><p class="muted">External models require provider credentials in production configuration.</p>
      </section>
      <section class="panel form" *ngIf="tab === 'guardrails'">
        <h3>Guardrails</h3><label>Operational policy<textarea [(ngModel)]="guardrails"></textarea></label>
      </section>
      <aside class="test">
        <header><div><b>Agent test</b><small>Safe sandbox only</small></div><span>Not live</span></header>
        <div class="testmsgs"><p class="user" *ngIf="prompt">{{ prompt }}</p><p class="bot" *ngIf="answer"><b>{{ agent.name }}</b>{{ answer }}</p></div>
        <textarea [(ngModel)]="prompt" placeholder="Test a customer message"></textarea><button class="primary" (click)="runTest()">Run test</button>
      </aside>
    </div>
    <qai-modal [open]="newOpen" title="New business assistant" (close)="newOpen = false"><form class="form" (ngSubmit)="create()"><label>Name<input [(ngModel)]="newAgent.name" name="name" required /></label><label>Role<input [(ngModel)]="newAgent.role" name="role" /></label><footer><button type="button" (click)="newOpen = false">Cancel</button><button class="primary" type="submit">Create agent</button></footer></form></qai-modal>`,
...
export class AiAgentsPage implements OnInit {
  ...
  agent: any = {
    ...
    role: "Support & Sales",
    instructions: "Resolve support questions from verified knowledge. Detect buying intent, collect qualification data, score leads and trigger approved revenue actions.",
    tone: "professional",
  ...
  ngOnInit() {
    this.data.list().subscribe((r) => {
      this.agents = r;
      if (r.length) { this.selectedId = r[0].id; this.selectAgent(); }
    });
  ...
  create() {
    ...
    this.data.create(this.newAgent).subscribe({
      next: (r) => { this.agents.push(r); this.selectedId = r.id; this.agent = { ...r }; this.newOpen = false; },
  ...
  runTest() {
    ...
    this.data.test(this.agent.id, this.prompt).subscribe({ next: (r) => (this.answer = r.message || "No response"), error: () => (this.answer = "Agent test failed. Check API/provider configuration.") });
  }
  desc(t: string) {
    const d: any = { SearchKnowledge: "Retrieve grounded answers", CreateLead: "Capture sales demand", CreateTicket: "Open a support case", CreateOpportunity: "Create pipeline revenue", BookMeeting: "Schedule a sales call", CallWebhook: "Execute external workflow" };
