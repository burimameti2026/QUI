...
@Component({
  ...
  template: ` <qai-page-header
      title="Users & Access"
      subtitle="Manage tenant users, roles, permissions and account status through QualifyAI Identity."
    ...
    <div class="callout warning" *ngIf="error">
      ...
      <div>
        <b>Identity service could not load users</b>
        <p>{{ error }}</p>
        <button (click)="load()">Try again</button>
      </div>
    </div>
    <section class="panel form" *ngIf="creating">
      <header>
        <div>
          <b>Create user</b
          ><span>The user receives access inside the current tenant.</span>
        </div>
      </header>
      <div class="form2">
    ...
    <section class="panel table-wrap">
      <div class="data-state" *ngIf="loading">Loading users…</div>
      <div class="data-state" *ngIf="!loading && !error && !rows.length">
        <b>No users found</b
        ><span
          >Create the first tenant user to assign roles and permissions.</span
        ><button class="primary" (click)="creating = true">Create user</button>
      </div>
      <table *ngIf="!loading && rows.length">
        ...
        <tbody>
          <tr *ngFor="let u of rows">
            ...
            <td>
              <span class="pill" [class.success]="u.isActive">{{
                u.isActive ? "Active" : "Disabled"
              }}</span>
            </td>
            <td>
              <button class="small" *ngIf="u.isActive" (click)="disable(u)">
                Disable</button
              ><button class="small" *ngIf="!u.isActive" (click)="enable(u)">
                Enable
              </button>
            </td>
          </tr>
...
export class UsersPage implements OnInit {
  ...
  create() {
    if (!this.canCreate) {
      this.error =
        "Complete every field and use a 10+ character password with uppercase, lowercase, number and special character.";
      return;
    }
    this.busy = true;
    this.error = "";
    this.users
      ...
      .subscribe({
        next: () => {
          this.creating = false;
          this.busy = false;
          this.form = { email: "", password: "", firstName: "", lastName: "" };
          this.load();
        },
        error: (e) => {
          this.error = this.apiError(e);
  ...
  private apiError(e: any) {
    return (
      ...
      e?.error?.error ||
      (e?.status
        ? `Identity API returned ${e.status}. Check identity-api logs for this request.`
        : "Identity API is unavailable.")
    );
  }
