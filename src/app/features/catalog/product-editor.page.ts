...
export class ProductEditorPage implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.api.get<Category[]>('renova/catalog/categories').subscribe({
      next: categories => {
        ...
        if (!this.editingId && !this.product.categoryId && categories.length) this.product.categoryId = categories[0].id;
  ...
  private loadProduct(): void {
    this.api.get<ProductDetails>(`renova/catalog/products/${this.editingId}`).subscribe({
      next: details => {
        ...
        this.product.categoryId = p.productCategoryId || '';
        this.product.brand = p.brand || 'Renova';
        this.product.shortDescription = p.shortDescription || '';
        this.product.description = p.description || '';
        this.product.benefits = p.keyBenefits || '';
        this.product.applications = p.applications || '';
        this.product.technicalSpecifications = p.technicalSpecifications || '';
        ...
        this.product.languages.en = this.product.languages.en || true;
        this.product.publish = details.publication?.status === 'Published' && details.publication?.isVisible;
      },
      error: err => this.error = this.message(err)
    });
  }
  ...
  save(): void {
    ...
    const body = {
      ...
      name: this.product.name.trim(), code: this.product.code.trim(), brand: this.product.brand?.trim() || null,
      shortDescription: this.product.shortDescription?.trim() || null, description: this.product.description?.trim() || null,
      keyBenefits: this.product.benefits?.trim() || null, applications: this.product.applications?.trim() || null,
      technicalSpecifications: this.product.technicalSpecifications?.trim() || null
    };

    const request = this.editingId
      ? this.api.put<any>(`renova/catalog/products/${this.editingId}`, body)
      : this.api.post<any>('renova/catalog/products', body);

    request.subscribe({
      next: product => {
        const productId = this.editingId || product?.id;
        if (!productId) { this.error = 'The API did not return a product id.'; this.saving = false; return; }
  ...
  private persistDetails(productId: string): void {
    const requests: Observable<unknown>[] = Object.entries(this.product.languages)
      ...
      .map(([language]) => this.api.put(`renova/catalog/products/${productId}/localizations/${language}`, {
        name: this.product.name, shortDescription: this.product.shortDescription || null, description: this.product.description || null,
        keyBenefits: this.product.benefits || null, applications: this.product.applications || null
      }));
  ...
  private finishProduct(productId: string): void {
    if (this.product.publish) {
      this.api.post(`renova/catalog/products/${productId}/publish`, { slug: null }).subscribe({
        next: () => this.complete(productId), error: err => { this.error = this.message(err); this.saving = false; }
      });
    } else if (this.editingId) {
      this.api.post(`renova/catalog/products/${productId}/unpublish`, {}).subscribe({
        next: () => this.complete(productId), error: err => { this.error = this.message(err); this.saving = false; }
      });
    } else {
