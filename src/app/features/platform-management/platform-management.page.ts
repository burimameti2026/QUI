...
@Component({
  ...
  template: `
    ...
    <section class="section">
      ...
      <div class="grid">
        <a routerLink="/platform/prepare-workspace" class="card featured blue"><span class="card-number">07</span><h3>Prepare Real Workspace</h3><p>Choose an industry or use case, start from a template, edit and select prospects, save the draft, then activate it as tenant-owned workspace data.</p><strong>Open workspace builder →</strong></a>
  ...
  styles: [`
    ...
    .hero { display:flex; justify-content:space-between; gap:28px; align-items:center; padding:28px; margin-bottom:14px; overflow:hidden; border:1px solid #d9e0ea; border-radius:16px; background:radial-gradient(circle at 88% 10%, #dbeafe 0, transparent 30%), linear-gradient(120deg, #f8fafc 0%, #eef2f7 58%, #f2effc 100%); box-shadow:0 7px 20px #17203309; }
    .hero h2 { margin:7px 0 8px; color:#172033; font-size:22px; letter-spacing:-.4px; }
    .hero p { max-width:720px; margin:0; color:#68758a; font-size:11px; line-height:1.65; }
    .primary { padding:11px 16px; border-radius:8px; text-decoration:none; font-size:10px; font-weight:700; background:#2563eb; color:#fff; white-space:nowrap; box-shadow:0 5px 12px #2563eb25; }
    ...
    .section-heading h3 { margin:5px 0 4px; color:#172033; font-size:14px; letter-spacing:-.1px; }
    .section-heading p { margin:0; color:#7b8798; font-size:10px; line-height:1.5; }
    .grid { display:grid; grid-template-columns:repeat(3,minmax(0,1fr)); gap:12px; }
    .card { --accent:#2563eb; --accent-soft:#dbeafe; position:relative; display:block; min-height:145px; box-sizing:border-box; padding:16px; overflow:hidden; text-decoration:none; color:inherit; border:1px solid #dfe5ed; border-radius:13px; background:linear-gradient(145deg,#fff 0%,#f4f6f9 100%); box-shadow:0 5px 16px #1720330a; transition:transform 160ms ease,border-color 160ms ease,box-shadow 160ms ease; }
    .card::before { content:''; position:absolute; inset:0 0 auto; height:3px; background:var(--accent); }
    .card::after { content:''; position:absolute; width:90px; height:90px; top:-52px; right:-38px; border-radius:50%; background:var(--accent-soft); opacity:.65; }
    .card:hover { transform:translateY(-2px); border-color:#c5d0df; background:linear-gradient(145deg,#fff 0%,#f1f4f8 100%); box-shadow:0 12px 26px #17203314; }
    .card.blue { --accent:#2563eb; --accent-soft:#dbeafe; }
    .card.violet { --accent:#7c3aed; --accent-soft:#ede9fe; }
    .card.green { --accent:#059669; --accent-soft:#d1fae5; }
    .card.amber { --accent:#d97706; --accent-soft:#fef3c7; }
    .card-number { position:relative; z-index:1; display:grid; width:31px; height:31px; place-items:center; margin-bottom:13px; border-radius:9px; color:var(--accent); background:var(--accent-soft); font-size:10px; font-weight:800; }
    .card h3 { position:relative; z-index:1; margin:0 0 6px; color:#172033; font-size:13px; }
    .card p { position:relative; z-index:1; margin:0; min-height:32px; color:#68758a; font-size:9px; line-height:1.55; }
    .card strong { position:relative; z-index:1; display:block; margin-top:12px; color:var(--accent); font-size:9px; }
    .featured { min-height:165px; }
