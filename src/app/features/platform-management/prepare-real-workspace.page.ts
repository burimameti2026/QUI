...
@Component({
  ...
  styles: [`
    ...
    .target-form label>select,.target-form label>input{margin-top:5px}
    ...
    .result-metrics .metric-top{font-size:11px}
      ...
      <section *ngIf="step===2" class="card setup-card">
        ...
        <div class="card-body">
          <div class="form content-grid target-form">
            ...
            <label>Industry
              <input [(ngModel)]="industry" placeholder="Logistics, SaaS, Manufacturing..."/>
            </label>
            <label>Region
              <input [(ngModel)]="region" placeholder="DACH, Europe, North America..."/>
            </label>
            <label>Countries
              <input [(ngModel)]="countries" placeholder="Germany, Austria, Switzerland"/>
            </label>
            <label>Workspace name
              <input [(ngModel)]="name" placeholder="Optional workspace name"/>
            </label>
            <label>Daily discovery limit
              <input type="number" [(ngModel)]="dailyDiscoveryLimit" min="1" max="100"/>
            </label>
      ...
      <section *ngIf="step===3" class="card setup-card">
        ...
        <div class="card-body">
          <div class="metric-grid result-metrics">
            <article class="metric"><div class="metric-top"><span class="metric-label">Agent</span></div><strong>{{result?.agentName || '-'}}</strong><small>{{result?.agentStatus || '-'}}</small></article>
            <article class="metric"><div class="metric-top"><span class="metric-label">Initial run</span></div><strong>{{result?.initialRunId || 'Already queued / not returned'}}</strong><small>Queued for autonomous discovery</small></article>
            <article class="metric"><div class="metric-top"><span class="metric-label">Target list</span></div><strong>{{result?.targetListId || 'Provisioned / pending'}}</strong><small>Qualified prospects route here</small></article>
            <article class="metric"><div class="metric-top"><span class="metric-label">Campaign</span></div><strong>{{result?.campaignId || 'Provisioned / pending'}}</strong><small>Delivery remains approval-controlled</small></article>
          </div>
          <section class="section lifecycle">
            <header class="section-header"><div><span class="eyebrow">PROVISIONING</span><h3>Workspace automation lifecycle</h3></div></header>
            <div class="list">
...
export class PrepareRealWorkspacePage {
  ...
  options:RealWorkspaceOptions|null=null;
  result:RealWorkspaceResult|null=null;
  step=1; useCase=''; templateKey=''; industry=''; region=''; countries=''; name='';
  dailyDiscoveryLimit=25; minimumScore=70; saving=false; status=''; error=false;
