...
@Component({
  ...
  template: `<qai-page-header title="Analytics & ROI" subtitle="Measure support efficiency, pipeline creation and Automation-influenced revenue."><button (click)="load()">↻ Refresh</button></qai-page-header>
    ...
    <div class="content-grid">
      <section class="card"><header><b>Revenue attribution</b><span>{{attribution.length}} records</span></header>
        <div class="table"><table><thead><tr><th>Model</th><th>Influenced revenue</th><th>Opportunity</th><th>Created</th></tr></thead><tbody><tr *ngFor="let x of attribution"><td>{{x.model}}</td><td><b>{{money(x.influencedRevenue)}}</b></td><td>{{x.opportunityId||'—'}}</td><td>{{x.createdAtUtc|date:'short'}}</td></tr></tbody></table></div>
      ...
      <section class="card"><header><b>Revenue automation engine</b><span>Current operating signals</span></header>
        <div class="notice"><div><b>{{d.hot||0}}</b><span>Hot leads</span></div><div><b>{{money(totalAttributed)}}</b><span>Attributed pipeline</span></div><div><b>{{conversion}}%</b><span>Pipeline won</span></div><div><b>{{d.aiConversations||0}}</b><span>Automated conversations</span></div></div>
      </section>
    </div>`,
