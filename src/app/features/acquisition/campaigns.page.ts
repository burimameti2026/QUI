...
@Component({
  ...
  styleUrls: ["./campaigns.page.css", "./campaigns-modern.css"],
})
export class CampaignsPage implements OnInit {
  ...
  get running(): number { return this.rows.filter((x) => x.status === 2).length; }
  get selectedList(): any { return this.lists.find((x) => x.id === this.form.targetListId); }
  get pendingMessages(): number { return this.messages.filter((x) => x.status === 0).length; }
  ...
  load(): void {
    ...
    this.data.campaigns().subscribe({
      ...
      error: (e) => { this.loading = false; this.error = this.apiError(e, "Campaigns could not be loaded."); },
    });
    this.data.targetLists().subscribe({
      next: (r) => {
        this.lists = r;
        const targetListId = this.route.snapshot.queryParamMap.get("targetListId");
        if (targetListId && r.some((x) => x.id === targetListId)) { this.form.targetListId = targetListId; this.show = true; }
      },
  ...
  openBuilder(): void { this.form = this.emptyForm(); this.builderStep = 1; this.error = ""; this.show = true; }
  openPipelineStep(step: number): void { this.form = this.emptyForm(); this.builderStep = step; this.error = ""; this.show = true; }
  showApprovalQueue(): void { this.approvalQueue?.nativeElement.scrollIntoView({ behavior: "smooth", block: "start" }); }
  openDemos(): void { void this.router.navigate(["/meetings"]); }
  openGuardrail(guardrail: any): void { this.selectedGuardrail = guardrail; this.guardrailOpen = true; }
  openGuardrailAction(): void {
    const guardrail = this.selectedGuardrail;
    this.guardrailOpen = false;
    if (guardrail?.target === "approval") { setTimeout(() => this.showApprovalQueue()); return; }
    if (guardrail?.route) void this.router.navigate([guardrail.route]);
  }
  next(): void { if (this.canContinue && this.builderStep < 4) this.builderStep++; }
  back(): void { if (this.builderStep > 1) this.builderStep--; }
  save(): void {
    if (!this.canContinue) return;
    this.busy = true;
    this.data.createCampaign(this.form).subscribe({
      next: (campaign) => { this.busy = false; this.rows.unshift(campaign); this.show = false; this.message = "Campaign created as draft. Review it, then start to queue approval-controlled messages."; },
      error: (error) => { this.busy = false; this.error = error?.error?.detail || "Campaign could not be created."; },
    });
  }
  start(campaign: any): void {
    this.data.startCampaign(campaign.id).subscribe({
      next: (result) => { campaign.status = result.status; this.load(); this.message = `${result.recipients} recipients enrolled; ${result.queued} first messages await approval.`; },
  ...
  requestApproval(message: any): void {
    this.data.requestApproval(message.id).subscribe({
      next: () => { message.approvalRequested = true; this.message = "Approval task created."; },
  ...
  approveAndSend(message: any): void {
    this.data.approveAndSend(message.id).subscribe({
      next: (result) => { this.load(); this.message = `Email accepted by provider: ${result.providerMessageId}`; },
  ...
  retryMessage(message: any): void {
    this.data.retryMessage(message.id).subscribe({
      next: (result) => { this.load(); this.message = `Retry accepted by provider: ${result.providerMessageId}`; },
      error: (e) => (this.error = this.apiError(e, "Email retry failed.")),
    });
  }
  status(value: number): string { return (["Draft", "Scheduled", "Running", "Paused", "Completed"][value] || String(value)); }
  messageStatus(value: number): string { return (["Queued", "Sent", "Delivered", "Replied", "Failed", "Suppressed"][value] || String(value)); }
  ...
  private apiError(error: any, fallback: string): string { return error?.error?.detail || error?.error?.error || (error?.status ? `${fallback} API returned ${error.status}.` : fallback); }
  private emptyForm(): any {
    return {
      name: "European logistics growth", targetListId: "", goal: "book-demo", senderName: "Sales team", senderEmail: "sales@company.com", startsAtUtc: null,
      steps: [
