...
styles:[`:host{display:block;min-width:0}.metrics{display:grid!important;grid-template-columns:repeat(5,minmax(0,1fr))!important;gap:var(--wl-kpi-gap,var(--wl-grid-gap,16px))!important;width:100%;min-width:0;box-sizing:border-box}.metric{appearance:none;min-width:0;min-height:var(--wl-kpi-height,112px);box-sizing:border-box;display:flex!important;flex-direction:column;justify-content:space-between;align-items:stretch;gap:10px;padding:var(--wl-kpi-padding,16px)!important;margin:0!important;background:var(--wl-kpi-bg,var(--wl-card-bg,#fff))!important;color:var(--wl-kpi-text,var(--wl-card-text,#475467))!important;border:1px solid var(--wl-kpi-border,var(--wl-card-border,#e3e9f1))!important;border-radius:var(--wl-kpi-radius,var(--wl-card-radius,12px))!important;box-shadow:var(--wl-kpi-shadow,var(--wl-card-shadow,0 2px 8px rgba(16,24,40,.06)))!important;text-align:left;font:inherit;cursor:default}.metric:hover{transform:none}.metric .top{display:flex;align-items:center;gap:8px;min-width:0;color:var(--wl-kpi-muted,var(--wl-card-muted,#667085));font-size:11px;font-weight:650;line-height:1.3;text-transform:uppercase;letter-spacing:.035em}.metric .icon{width:26px;height:26px;flex:0 0 26px;display:grid;place-items:center;border-radius:8px;background:var(--wl-kpi-icon-bg,var(--wl-accent-soft,#edf4ff));color:var(--wl-kpi-icon,var(--wl-accent,#2563eb));font-size:13px}.metric strong{display:block;margin:0;color:var(--wl-kpi-value,var(--wl-card-title,#172033));font-size:25px;font-weight:780;line-height:1;letter-spacing:-.035em}.metric small{display:flex;align-items:center;justify-content:space-between;gap:8px;min-height:16px;color:var(--wl-kpi-muted,var(--wl-card-muted,#667085));font-size:10px;line-height:1.35}.metric small i{font-style:normal;color:var(--wl-kpi-accent,var(--wl-accent,#2563eb));font-weight:700}@media(max-width:1100px){.metrics{grid-template-columns:repeat(3,minmax(0,1fr))!important}}@media(max-width:760px){.metrics{grid-template-columns:repeat(2,minmax(0,1fr))!important}}@media(max-width:520px){.metrics{grid-template-columns:1fr!important}}`]})
...
@Component({selector:'qai-ui-metrics',standalone:true,imports:[CommonModule],template:`<div class="metrics" [attr.data-template]="template"><button type="button" *ngFor="let item of items; let i = index" class="metric" (click)="action.emit(item.action?.route||'')"><span class="top"><span class="icon">{{item.icon||'•'}}</span><span>{{item.label}}</span></span><strong>{{item.value}}</strong><small>{{item.subtitle}}<i *ngIf="item.action">→</i></small></button></div>`,styles:[`:host{display:block;min-width:0}`]})
