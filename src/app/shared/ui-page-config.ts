...
export type UiTemplateId =
  ...
  | 'list-01' | 'grid-01' | 'table-01' | 'text-01';

export interface UiBinding { source: string; fallback?: string | number; }
export interface UiPageAction { label: string; route: string; variant?: 'primary' | 'secondary' | 'quiet'; }
...
export const UI_COMPONENT_REGISTRY: Record<UiPageComponentType, {
  ...
  header: { templates: ['header-01'], configurable: ['title', 'subtitle', 'actions'] },
  ...
  grid: { templates: ['grid-01'], configurable: ['columns', 'children'] },
  text: { templates: ['text-01'], configurable: ['title', 'text'] }
};
...
export const DEFAULT_DASHBOARD_PAGE_CONFIG: UiPageConfig = {
  ...
  sections: [
    ...
    { id: 'kpis', columns: 1, components: [{ id: 'operating-kpis', type: 'metrics', template: 'kpi-01' }] },
