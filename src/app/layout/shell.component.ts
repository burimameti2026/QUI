...
@Component({selector:"qai-shell",standalone:true,imports:[CommonModule,FormsModule,RouterOutlet,RouterLink,RouterLinkActive,AdminStaticI18nDirective],template:`<div class="shell"><aside><div class="brand"><i class="brand-mark">Q</i><span>Qualify</span><strong>AI</strong><small>ENTERPRISE</small></div><div class="workspace"><i>{{initials(workspaceName)}}</i><div><b>{{workspaceName}}</b><span>{{i18n.t(runtime.runtime()?.plan || session?.licensePlan || 'Licensed')}} workspace</span></div></div><nav class="department-nav"><ng-container *ngFor="let group of visibleGroups"><button type="button" class="department-toggle" [class.expanded]="isExpanded(group)" (click)="toggleGroup(group)" [attr.aria-expanded]="isExpanded(group)"><span class="department-title"><i>{{groupIcon(group)}}</i><b>{{i18n.t(group)}}</b></span><em>⌄</em></button><div class="department-items" *ngIf="isExpanded(group)"><a *ngFor="let item of navBy(group)" [routerLink]="item.url" routerLinkActive="active"><span>{{item.icon}}</span>{{i18n.t(item.label)}}</a></div></ng-container></nav></aside><main><header class="app-header"><div class="header-search-wrap"><label class="global-search"><span>⌕</span><input [(ngModel)]="query" [placeholder]="i18n.t('Search pages and modules')"/><kbd>Ctrl K</kbd></label><a class="portal-link" routerLink="/renova/portal">↗ {{i18n.t('Public Renova portal')}}</a><div class="admin-language"><span>◎</span><select [ngModel]="i18n.language()" (ngModelChange)="setLanguage($event)"><option *ngFor="let language of i18n.languages" [value]="language.code">{{language.label}}</option></select></div><section class="header-search-results" *ngIf="query.trim()"><button type="button" *ngFor="let item of searchResults" (click)="go(item.url)"><i>{{item.icon}}</i><span><b>{{i18n.t(item.label)}}</b><small>{{i18n.t(item.group)}}</small></span></button></section></div></header><section class="page" qaiAdminStaticI18n><router-outlet/></section></main></div>`,styles:[`.department-nav{padding:8px 10px 20px}.department-toggle{display:flex;align-items:center;justify-content:space-between;width:100%;min-height:42px;margin:3px 0;padding:0 9px;border:0;border-radius:9px;background:transparent;color:#68758a;text-align:left;cursor:pointer}.department-toggle:hover,.department-toggle.expanded{background:#f4f6f9;color:#172033}.department-title{display:flex;align-items:center;gap:9px;min-width:0}.department-title i{display:grid;width:27px;height:27px;place-items:center;border:1px solid #dfe5ed;border-radius:7px;background:#fff;color:#2563eb;font-style:normal;font-size:11px}.department-title b{font-size:10px;letter-spacing:.04em}.department-toggle em{font-style:normal;font-size:13px;transition:transform .18s ease}.department-toggle.expanded em{transform:rotate(180deg)}.department-items{padding:2px 0 7px 12px;border-left:1px solid #e5eaf1;margin-left:22px}.department-items a{display:flex;align-items:center;gap:9px;min-height:35px;margin:2px 0;padding:0 9px;border-radius:8px;color:#526078;text-decoration:none;font-size:11px;font-weight:650}.department-items a:hover{background:#f6f8fb;color:#172033}.department-items a.active{background:#eef4ff;color:#2563eb;box-shadow:inset 3px 0 0 #2563eb}.department-items a span{width:18px;text-align:center;font-size:12px}@media(max-width:850px){.department-title b{font-size:9px}.department-items a{font-size:10px}}`]})
export class ShellComponent {
 ...
 private expandedGroups=new Set<string>(['COMMAND CENTER']);
