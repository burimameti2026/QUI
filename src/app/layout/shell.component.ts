...
export class ShellComponent {
 ...
 {group:'PLATFORM MANAGEMENT',label:'Platform Overview',url:'/platform',icon:'◉',module:'platform',permission:'platform.manage'}];
 constructor(){this.runtime.load().subscribe({error:()=>{}});}
 get session(){return this.auth.session();} get workspaceName(){return this.session?.tenantSlug||'Workspace';} get visibleGroups(){return this.groups.filter(g=>this.navBy(g).length>0);} get searchResults(){const q=this.query.trim().toLowerCase();return this.nav.filter(i=>this.allowed(i)&&(!q||`${i.label} ${i.group} ${i.module}`.toLowerCase().includes(q))).slice(0,9);} navBy(group:string){return this.nav.filter(i=>i.group===group&&this.allowed(i));} allowed(item:NavigationItem){return item.module==='platform'?this.auth.hasPermission(item.permission):(item.module==='billing'||this.runtime.hasModule(item.module))&&this.auth.hasPermission(item.permission);} go(url:string){void this.router.navigate([url]);this.query='';} initials(v:string){return v.split(/\s+|@/).filter(Boolean).map(x=>x[0]).join('').slice(0,2).toUpperCase()||'U';}
}
