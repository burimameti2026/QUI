...
export class ShellComponent {
 readonly auth=inject(AuthService); readonly runtime=inject(TenantRuntimeService); private readonly router=inject(Router); query='';
 ...
 allowed(item:NavigationItem){if(item.module==='core')return this.runtime.isActive();if(item.module==='platform')return this.auth.hasPermission('system.admin');return this.runtime.hasModule(item.module)&&this.auth.hasPermission(item.permission);}
