...
interface AdvisorMessage { role: 'user' | 'assistant'; text: string; suggestions?: string[]; nextAction?: string; field?: string; tool?: string; toolInput?: string; requiresApproval?: boolean; approvalState?: 'pending' | 'approved' | 'rejected'; }
  ...
  template: `
    ...
    <section *ngIf="open" class="advisor-panel" aria-label="AI Advisor">
      ...
      <div class="advisor-messages">
        <div *ngFor="let item of messages" class="message" [class.user]="item.role==='user'"><div class="message-bubble">{{ item.text }}</div><div *ngIf="item.suggestions?.length" class="suggestions"><button *ngFor="let suggestion of item.suggestions" type="button" (click)="useSuggestion(suggestion)">{{ suggestion }}</button></div><div *ngIf="item.nextAction" class="next-action"><span>NEXT</span>{{ item.nextAction }}</div><div *ngIf="item.requiresApproval && item.approvalState==='pending'" class="approval"><strong>Approval required</strong><small>This action will change workspace data. Review the proposed action before it runs.</small><div><button type="button" (click)="approve(item)">Approve</button><button type="button" (click)="reject(item)">Reject</button></div></div><div *ngIf="item.approvalState==='approved'" class="approval done">Approved and executed.</div><div *ngIf="item.approvalState==='rejected'" class="approval done">Action rejected.</div></div>
        <div *ngIf="loading" class="typing"><span></span><span></span><span></span> Thinking…</div><div *ngIf="error" class="advisor-error">{{ error }}</div>
      </div>
      <div class="quick-actions"><button type="button" (click)="ask('What should I do next?')">What next?</button><button type="button" (click)="ask('What should I improve here?')">Improve this</button><button type="button" (click)="analyzeBusiness()">Choose my pack</button><button type="button" (click)="ask('I am not sure what to configure here. Analyze the current context, tell me what this section is for, recommend what I should do, and give me exact text I can use.')">Help me write</button></div>
...
export class AiAdvisorComponent implements OnInit {
  ...
  send(){ const text=this.draft.trim(); if(!text||this.loading)return; this.messages.push({role:'user',text}); this.draft=''; this.loading=true; this.error=''; this.advisor.runAgent(text).subscribe({next:(r:any)=>{this.loading=false;this.messages.push({role:'assistant',text:r?.message||'I could not generate a suggestion.',suggestions:r?.suggestions,nextAction:r?.nextAction,field:r?.field,tool:r?.tool,toolInput:r?.toolInput,requiresApproval:r?.requiresApproval,approvalState:r?.requiresApproval?'pending':undefined});},error:(e:any)=>{this.loading=false;this.error=e?.error?.detail||e?.error?.error||'AI Advisor is temporarily unavailable.';}}); }
  ...
  approve(item: AdvisorMessage){ if(!item.tool || !item.toolInput || this.loading)return; this.loading=true; this.error=''; this.advisor.executeTool(item.tool,item.toolInput).subscribe({next:()=>{this.loading=false;item.approvalState='approved';},error:(e:any)=>{this.loading=false;this.error=e?.error?.detail||e?.error?.error||'The approved action could not be executed.';}}); }
  reject(item: AdvisorMessage){ item.approvalState='rejected'; }
  getContext(){ return this.advisor.context(); }
  analyzeBusiness(){ this.ask('I am not sure which Industry Pack I should use. Analyze my current business context and available packs. If one clearly fits, recommend it and explain why. If none fits, tell me what new pack we should create. If you do not have enough information, ask me the minimum questions needed.'); }
}