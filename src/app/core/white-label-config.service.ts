...
export interface WhiteLabelGlobalTokens {
  ...
  headerGap: number;
  pageHeaderTitleSize: number;
  pageHeaderGap: number;
  cardRadius: number;
  cardPadding: number;
  cardGap: number;
  cardShadow: string;
...
export interface WhiteLabelStyles {
  global: WhiteLabelGlobalTokens;
  header: WhiteLabelStyle;
  grid: WhiteLabelStyle;
  kpis: WhiteLabelStyle;
  cards: WhiteLabelStyle;
  buttons: WhiteLabelStyle;
  text: WhiteLabelStyle;
  steps: WhiteLabelStyle;
  lists: WhiteLabelStyle;
  tables: WhiteLabelStyle;
  badges: WhiteLabelStyle;
  forms: WhiteLabelStyle;
  tabs: WhiteLabelStyle;
  dataGrid: WhiteLabelStyle;
  navigation: WhiteLabelStyle;
  modals: WhiteLabelStyle;
  notices: WhiteLabelStyle;
}
...
export interface WhiteLabelSection {
  id: string;
  kind: string;
  label: string;
  columns: number;
...
const DEFAULT_GLOBAL_TOKENS: WhiteLabelGlobalTokens = {
...
const DEFAULT_APPEARANCE: WhiteLabelStyle = {
  ...
  buttonBackgroundColor: "#f5500bdb",
...
const DEFAULT_STYLES: WhiteLabelStyles = {
  ...
  text: { ...DEFAULT_APPEARANCE, headerColor: "#ffffff", height: 0, radius: 0 },
  steps: {
    ...DEFAULT_APPEARANCE,
    headerColor: "#ffffff",
    height: 0,
...
export class WhiteLabelConfigService {
  ...
  save(sections: WhiteLabelSection[]): void {
    const normalized = this.normalize(sections);
    localStorage.setItem(STORAGE_KEY, JSON.stringify(normalized));
    this.layout.set(normalized);
    this.apply(normalized);
  }
  loadStyles(): WhiteLabelStyles {
    try {
  ...
  saveStyles(styles: WhiteLabelStyles): WhiteLabelStyles {
    ...
    localStorage.setItem(STYLE_KEY, JSON.stringify(normalized));
    this.styles.set(normalized);
    this.apply(this.layout(), normalized);
    return normalized;
  }
  apply(sections = this.layout(), styles = this.styles()): void {
    const root = document.documentElement;
    const wlGlobal = styles.global;
    this.set(root, "--wl-bg", wlGlobal.appBackground);
    this.set(root, "--wl-panel", wlGlobal.surface);
    this.set(root, "--wl-line", wlGlobal.line);
    this.set(root, "--wl-text", wlGlobal.text);
    this.set(root, "--wl-muted", wlGlobal.muted);
    this.set(root, "--wl-blue", wlGlobal.accent);
    this.set(root, "--wl-nav", wlGlobal.nav);
    this.set(root, "--wl-nav-panel", wlGlobal.navPanel);
    this.set(root, "--wl-nav-border", wlGlobal.navBorder);
    this.set(root, "--wl-nav-muted", wlGlobal.navMuted);
    this.set(root, "--wl-nav-hover", wlGlobal.navHover);
    this.set(root, "--wl-nav-accent", wlGlobal.navAccent);
    this.set(root, "--wl-green", wlGlobal.success);
    this.set(root, "--wl-red", wlGlobal.danger);
    this.set(root, "--wl-amber", wlGlobal.warning);
    this.set(root, "--wl-info", wlGlobal.info);
    this.set(root, "--wl-surface-muted", wlGlobal.surfaceMuted);
    this.set(root, "--wl-input-placeholder", wlGlobal.inputPlaceholder);
    this.set(root, "--wl-table-header", wlGlobal.tableHeader);
    this.set(root, "--wl-table-line", wlGlobal.tableLine);
    this.set(root, "--wl-accent-soft", wlGlobal.accentSoft);
    this.set(root, "--wl-accent-border", wlGlobal.accentBorder);
    this.set(root, "--wl-avatar-bg", wlGlobal.avatarBg);
    this.set(root, "--wl-avatar-text", wlGlobal.avatarText);
    this.set(root, "--wl-page-max-width", wlGlobal.pageMaxWidth + "px");
    this.set(root, "--wl-page-padding-x", wlGlobal.pagePaddingX + "px");
    this.set(root, "--wl-page-padding-y", wlGlobal.pagePaddingY + "px");
    this.set(
      root,
      "--wl-page-padding-bottom",
      wlGlobal.pagePaddingBottom + "px",
    );
    this.set(root, "--wl-sidebar-width", wlGlobal.sidebarWidth + "px");
    this.set(root, "--wl-sidebar-padding-x", wlGlobal.sidebarPaddingX + "px");
    this.set(root, "--wl-sidebar-padding-y", wlGlobal.sidebarPaddingY + "px");
    this.set(root, "--wl-header-height", wlGlobal.headerHeight + "px");
    this.set(root, "--wl-header-padding-x", wlGlobal.headerPaddingX + "px");
    this.set(root, "--wl-header-gap", wlGlobal.headerGap + "px");
    this.set(
      root,
      "--wl-page-header-title-size",
      wlGlobal.pageHeaderTitleSize + "px",
    );
    this.set(root, "--wl-page-header-gap", wlGlobal.pageHeaderGap + "px");
    this.set(root, "--wl-card-radius", wlGlobal.cardRadius + "px");
    this.set(root, "--wl-card-padding", wlGlobal.cardPadding + "px");
    this.set(root, "--wl-card-gap", wlGlobal.cardGap + "px");
    this.set(root, "--wl-card-shadow", wlGlobal.cardShadow);
    this.set(root, "--wl-control-height", wlGlobal.controlHeight + "px");
    this.set(root, "--wl-control-radius", wlGlobal.controlRadius + "px");
    this.set(root, "--wl-modal-radius", wlGlobal.modalRadius + "px");
    this.set(root, "--wl-modal-shadow", wlGlobal.modalShadow);
    ...
    this.set(
      root,
      "--wl-accent-hover",
      card.buttonHoverBackgroundColor || this.darken(card.accentColor),
    );
    this.set(root, "--wl-accent-soft", wlGlobal.accentSoft);
    this.set(root, "--wl-grid-gap", (grid.gap || 12) + "px");
    this.set(root, "--wl-grid-columns", "2");
    this.set(root, "--wl-header-bg", header.headerColor);
    this.set(root, "--wl-header-border", header.borderColor);
    this.set(root, "--wl-header-title", header.titleColor);
    this.set(root, "--wl-header-text", header.textColor);
    this.set(root, "--wl-header-muted", header.mutedTextColor);
    this.set(root, "--wl-header-height", (header.height || 64) + "px");
    this.set(root, "--wl-header-padding", (header.padding || 14) + "px");
    this.set(root, "--wl-header-gap", (header.gap || 20) + "px");
    ...
    this.set(root, "--wl-card-muted", card.mutedTextColor);
    this.set(root, "--wl-card-accent", card.accentColor);
    this.set(root, "--wl-card-radius", card.radius + "px");
    this.set(root, "--wl-card-padding", (card.padding || 16) + "px");
    this.set(root, "--wl-card-gap", (card.gap || 12) + "px");
    this.set(root, "--wl-card-font-size", (card.fontSize || 14) + "px");
    this.set(
      root,
      "--wl-card-shadow",
      card.shadow || "0 4px 14px rgba(15,47,104,.05)",
    );
    ["button-01", "button-02", "button-03", "button-04"].forEach(
      (template, index) => {
        const a = buttonsStyle;
        const n = index + 1;
        this.set(
          root,
          "--wl-button-" + n + "-bg",
          a.buttonBackgroundColor || a.accentColor,
        );
        this.set(
          root,
          "--wl-button-" + n + "-text",
          a.buttonTextColor || "#ffffff",
        );
        this.set(
          root,
          "--wl-button-" + n + "-border",
          a.buttonBorderColor || a.accentColor,
        );
        this.set(
          root,
          "--wl-button-" + n + "-hover",
          a.buttonHoverBackgroundColor ||
            this.darken(a.buttonBackgroundColor || a.accentColor),
        );
      },
    );
    ...
    this.set(root, "--wl-kpi-4", kpis.accentColor);
    this.set(root, "--wl-kpi-5", kpis.accentColor);
    this.set(root, "--wl-kpi-bg", kpis.surfaceColor);
    this.set(root, "--wl-kpi-header", kpis.headerColor);
    this.set(root, "--wl-kpi-border", kpis.borderColor);
    this.set(root, "--wl-kpi-title", kpis.titleColor);
    this.set(root, "--wl-kpi-text", kpis.textColor);
    this.set(root, "--wl-kpi-muted", kpis.mutedTextColor);
    this.set(root, "--wl-kpi-radius", kpis.radius + "px");
    this.set(root, "--wl-kpi-padding", (kpis.padding || 16) + "px");
    this.set(
      root,
      "--wl-kpi-shadow",
      kpis.shadow || "0 4px 14px rgba(15,47,104,.05)",
    );
    this.set(
      root,
      "--wl-button-global-bg",
      buttonsStyle.buttonBackgroundColor || buttonsStyle.accentColor,
    );
    this.set(
      root,
      "--wl-button-global-text",
      buttonsStyle.buttonTextColor || "#fff",
    );
    this.set(
      root,
      "--wl-button-global-border",
      buttonsStyle.buttonBorderColor || buttonsStyle.accentColor,
    );
    this.set(
      root,
      "--wl-button-global-hover",
      buttonsStyle.buttonHoverBackgroundColor ||
        this.darken(buttonsStyle.accentColor),
    );
    this.set(root, "--wl-button-radius", buttonsStyle.radius + "px");
    this.set(
      root,
      "--wl-button-padding-y",
      Math.max(6, Math.round((buttonsStyle.padding || 10) * 0.6)) + "px",
    );
    this.set(
      root,
      "--wl-button-padding-x",
      Math.max(8, Math.round((buttonsStyle.padding || 10) * 0.9)) + "px",
    );
    this.set(
      root,
      "--wl-button-font-size",
      (buttonsStyle.fontSize || 10) + "px",
    );
    this.set(
      root,
      "--wl-button-font-weight",
      String(buttonsStyle.fontWeight || 700),
    );
    this.set(root, "--wl-text-surface", text.surfaceColor);
    this.set(root, "--wl-text-padding", (text.padding || 18) + "px");
    this.set(root, "--wl-text-title-size", (text.fontSize || 14) + "px");
    this.set(
      root,
      "--wl-text-body-size",
      Math.max(10, (text.fontSize || 14) - 3) + "px",
    );
    this.set(root, "--wl-text-shadow", text.shadow || "none");
    this.set(root, "--wl-text-border", text.borderColor);
    this.set(root, "--wl-text-radius", (text.radius || 0) + "px");
    this.set(root, "--wl-text-title", text.titleColor);
    this.set(root, "--wl-text-body", text.textColor);
    this.set(root, "--wl-text-muted", text.mutedTextColor);
    this.set(root, "--wl-steps-bg", steps.surfaceColor);
    this.set(root, "--wl-steps-header", steps.headerColor);
    this.set(root, "--wl-steps-border", steps.borderColor);
    this.set(root, "--wl-steps-title", steps.titleColor);
    ...
    this.set(root, "--wl-table-title", tables.titleColor);
  ...
  private normalizeStyles(styles: any): WhiteLabelStyles {
    const source = styles || {};
    const out: any = {
      global: { ...DEFAULT_GLOBAL_TOKENS, ...(source.global || {}) },
    };
    for (const key of Object.keys(DEFAULT_STYLES)) {
      out[key] = {
        ...DEFAULT_APPEARANCE,
        ...(DEFAULT_STYLES[key as keyof WhiteLabelStyles] || {}),
        ...(source[key] || {}),
      };
    }
    // Migrate the previous blue visual system to the current orange/neutral product language.
    const ORANGE = "#f5500bdb";
    const ORANGE_HOVER = "#000000";
    const ORANGE_SOFT = "#fff4df";
    const ORANGE_BORDER = "#ffd08a";
    if (out.global.accent === "#2563eb") out.global.accent = ORANGE;
    if (out.global.info === "#2563eb") out.global.info = ORANGE;
    if (out.global.accentSoft === "#edf4ff")
      out.global.accentSoft = ORANGE_SOFT;
    if (out.global.accentBorder === "#cfe0ff")
      out.global.accentBorder = ORANGE_BORDER;
    if (out.global.avatarBg === "#dbeafe") out.global.avatarBg = "#fff0cf";
    if (out.global.avatarText === "#1d4ed8") out.global.avatarText = "#b45309";
    for (const key of Object.keys(DEFAULT_STYLES)) {
      const style = out[key];
      if (!style) continue;
      if (style.accentColor === "#2563eb" || style.accentColor === "#4f46e5")
        style.accentColor = ORANGE;
      if (style.buttonBackgroundColor === "#2563eb")
        style.buttonBackgroundColor = ORANGE;
      if (style.buttonBorderColor === "#2563eb")
        style.buttonBorderColor = ORANGE;
      if (style.buttonHoverBackgroundColor === "#1d4ed8")
        style.buttonHoverBackgroundColor = ORANGE_HOVER;
    ...
    if (
      ...
      out.cards.headerColor = "#555555";
    if (
      out.cards?.titleColor === "#101828" ||
      out.cards?.titleColor === "#344054" ||
      out.cards?.titleColor === "#667085"
    )
      out.cards.titleColor = ORANGE;
    ...
    if (
      out.badges?.headerColor === "#edf4ff" ||
      out.badges?.headerColor === "#eef2ff"
    )
      out.badges.headerColor = ORANGE_SOFT;
    if (
      out.badges?.accentColor === "#2563eb" ||
      out.badges?.accentColor === "#4f46e5"
    )
      out.badges.accentColor = ORANGE;
    return out as WhiteLabelStyles;
  }
  private normalize(sections: any[]): WhiteLabelSection[] {
    return sections.map((section) => ({
      ...section,
      items: Array.isArray(section.items)
        ? section.items.map((item: any) => {
            const { appearance, ...clean } = item || {};
            return clean;
          })
        : [],
    }));
  }
  private enabled(sections: WhiteLabelSection[], kind: string) {
    return (
      sections
        .find((x) => x.kind === kind)
        ?.items?.filter((x) => x.enabled !== false) || []
    );
  }
  private firstEnabled(sections: WhiteLabelSection[], kind: string) {
    return this.enabled(sections, kind)[0];
  }
  private set(root: HTMLElement, name: string, value: string) {
    root.style.setProperty(name, value);
  }
  private darken(hex: string) {
    if (!/^#[0-9a-fA-F]{6}$/.test(hex)) return "#1d4ed8";
    const r = Math.max(0, parseInt(hex.slice(1, 3), 16) - 25),
      g = Math.max(0, parseInt(hex.slice(3, 5), 16) - 25),
      b = Math.max(0, parseInt(hex.slice(5, 7), 16) - 25);
