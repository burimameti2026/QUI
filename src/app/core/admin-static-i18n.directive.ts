...
export class AdminStaticI18nDirective implements OnDestroy {
  ...
  private translateComponentInputs(elements: HTMLElement[]): void {
    ...
    for (const element of elements) {
      ...
      if (!originals) {
        originals = new Map();
        this.propertyOriginals.set(element, originals);
      }
      ...
      for (const prop of scalarProps) {
        ...
        if (resolved !== source && (element as any)[prop] !== resolved) {
          (element as any)[prop] = resolved;
        }
      }
    }
  }
  ...
  private translateValue(value: string): string {
    ...
    const universal = adminUniversalText(value, language);
    if (universal !== value) return universal;

    const leads = adminLeadsText(value, language);
    if (leads !== value) return leads;

    const dashboard = adminDashboardText(value, language);
    if (dashboard !== value) return dashboard;
