...
export class AdminStaticI18nDirective implements OnDestroy {
  ...
  constructor(){
    // Observe structure changes only. Watching characterData makes every Angular
    // text update enter the global translation pipeline and can create a large
    // change-detection/DOM-work loop across the whole page.
  ...
  private translateValue(value:string):string{
    ...
    if(dynamicKpi){
      const count=dynamicKpi[1];const suffix=dynamicKpi[2].toLowerCase();
      ...
      const key=suffixKeys[suffix];const translatedSuffix=key?this.translateValue(key).replace(/^0\s*/,''):suffix;
  ...
  private translateCompound(value:string,language:AdminLanguage):string{
    ...
    if(match){const key=match[2].toLowerCase()==='total'?'Total':'Pending';const translated=adminPageInteriorDynamicText(key,language);if(translated!==key)return `${match[1]} ${translated}`}
    ...
    if(match){const indexed={en:'Indexed',mk:'Индексирани',sq:'Të indeksuara',de:'Indiziert'}[language];const chunks={en:'chunks',mk:'делови',sq:'pjesë',de:'Abschnitte'}[language];return `${indexed} ${match[1]} ${chunks}.`}
    match=value.match(/^Delete\s+(.+)\?$/i);
    if(match){const deleteLabel=adminPageInteriorGlobalText('Delete',language);if(deleteLabel!=='Delete')return `${deleteLabel} ${match[1]}?`}
    return value;
  }
