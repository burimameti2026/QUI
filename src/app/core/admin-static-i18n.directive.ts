...
export class AdminStaticI18nDirective implements OnDestroy {
  ...
  private scheduleMutations(records: MutationRecord[]): void {
    ...
    // Only translate newly-added/changed nodes. Do not observe attributes: translation
    // itself changes placeholder/title/aria-label and can otherwise create an endless loop.
    queueMicrotask(() => {
      if (this.translating) return;
      const roots = new Set<Node>();
      for (const record of records) {
        if (record.type === 'childList') record.addedNodes.forEach(node => roots.add(node));
        else if (record.type === 'characterData' && record.target) roots.add(record.target);
      }
  ...
  private translateRoots(roots: Node[]): void {
    ...
    try {
      ...
      for (const root of roots) {
        ...
        if (element.matches('*')) element.querySelectorAll('*').forEach(el => elements.push(el as HTMLElement));
      }
  ...
  private translateValue(value: string): string {
    ...
    if (dynamicKpi) { const count = dynamicKpi[1]; const suffix = dynamicKpi[2].toLowerCase(); const suffixKeys: Record<string,string> = { selected:'0 selected','high priority':'0 high priority','verified accounts':'Verified accounts','hot prospects':'Hot prospects','active campaigns':'Active campaigns',replies:'Replies','demo ready':'Demo ready' }; const key=suffixKeys[suffix]; const translatedSuffix=key?this.translateValue(key).replace(/^0\s*/, ''):suffix; if(translatedSuffix!==suffix)return `${count} ${translatedSuffix}`; }
  ...
  private translateCompound(value: string, language: AdminLanguage): string {
    let match=value.match(/^(\d+)\s+campaigns\s+·\s+(\d+)\s+running$/i); if(match)return `${match[1]} ${adminPageInteriorDynamicText('Campaigns',language)} · ${match[2]} ${adminPageInteriorDynamicText('Active',language).toLowerCase()}`;
    match=value.match(/^(\d+)\s+(total|pending)$/i); if(match){const key=match[2].toLowerCase()==='total'?'Total':'Pending';const translated=adminPageInteriorDynamicText(key,language);if(translated!==key)return `${match[1]} ${translated}`;}
    match=value.match(/^Showing\s+(\d+)\s*[–-]\s*(\d+)\s+of\s+(\d+)$/i); if(match)return `${adminPageInteriorDynamicText('Showing',language)} ${match[1]}–${match[2]} ${adminPageInteriorDynamicText('of',language)} ${match[3]}`;
    match=value.match(/^Page\s+(\d+)\s+of\s+(\d+)$/i); if(match)return `${adminPageInteriorDynamicText('Page',language)} ${match[1]} ${adminPageInteriorDynamicText('of',language)} ${match[2]}`;
    match=value.match(/^Indexed\s+(\d+)\s+chunks\.?$/i); if(match){const indexed={en:'Indexed',mk:'Индексирани',sq:'Të indeksuara',de:'Indiziert'}[language];const chunks={en:'chunks',mk:'делови',sq:'pjesë',de:'Abschnitte'}[language];return `${indexed} ${match[1]} ${chunks}.`;}
    match=value.match(/^Delete\s+(.+)\?$/i); if(match){const deleteLabel=adminPageInteriorGlobalText('Delete',language);if(deleteLabel!=='Delete')return `${deleteLabel} ${match[1]}?`;}
    return value;
  }
